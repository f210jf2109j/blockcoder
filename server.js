...
io.on("connection", (socket) => {
  socket.on("host-room", ({ room, offer }) => {
    ...
    rooms.set(code, { host: socket.id, offer });
    socket.join(code);
    socket.emit("room-ready", { room: code });
  });
  ...
  socket.on("join-room", ({ room }) => {
    ...
    if (!data) {
      socket.emit("room-error", "Room not found. Check the code or have the host create a new room.");
      return;
    }
    socket.join(code);
    socket.emit("offer", { room: code, offer: data.offer });
  });
